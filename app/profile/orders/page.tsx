...
export default function MyOrdersPage() {
  ...
  const [reschedulingOrder, setReschedulingOrder] = useState<Order | null>(
    null,
  );
  ...
  const slotQueryParams = useMemo(() => {
    const vehicle = reschedulingOrder
      ? vehicles.find((v) => (v._id || v.id) === reschedulingOrder.vehicleId)
      : undefined;
    const vehicleTypeId = vehicle?.vehicleTypeId || '';
    if (!reschedulingOrder || !vehicleTypeId || !rescheduleDate) {
      return {
        serviceTypeId: '',
        vehicleTypeId: '',
        from: '',
  ...
  }, [reschedulingOrder, vehicles, rescheduleDate]);
  ...
  const handleRescheduleSubmit = async () => {
    ...
    try {
      ...
      const slotsRes = await getAvailableSlotsForReschedule(
        reschedulingOrder.serviceTypeId,
        rescheduleSlot,
      );
      ...
      toast.error(
        (error as { response?: { data?: { message?: string } } }).response?.data
          ?.message || 'Không thể dời lịch. Vui lòng thử lại!',
      );
    }
  };

  // Quick helper to fetch active shift covering the slot
  const getAvailableSlotsForReschedule = async (
  ...
  const renderLicensePlate = (plate: string) => {
    ...
    return (
      <div className='inline-flex flex-col items-center justify-center border border-slate-300 rounded-md px-2.5 py-0.5 bg-slate-50 font-mono shadow-xs text-[10px] leading-none shrink-0 scale-95 origin-left'>
        <div className='text-[7px] text-slate-400 font-sans pb-0.5 leading-none'>
          VIỆT NAM
        </div>
        <span className='font-bold text-slate-800 tracking-wide leading-none'>
          {plate}
        </span>
      </div>
    );
            ...
            <CardContent className='p-6'>
              <div className='flex items-center justify-between border-b border-slate-100 pb-3 mb-5'>
                <h3 className='font-heading text-base font-bold text-foreground flex items-center gap-2'>
                  <Calendar className='w-5 h-5 text-primary' /> Đổi lịch hẹn rửa
                  xe
                </h3>
                <button
                  onClick={() => {
              ...
              <div className='space-y-4'>
                <div className='bg-slate-50 p-3 rounded-xl border border-slate-100 space-y-1'>
                  ...
                  <span className='text-[10px] text-slate-400 font-semibold block'>
                    Lịch cũ:{' '}
                    {new Date(reschedulingOrder.scheduledAt).toLocaleTimeString(
                      'vi-VN',
                      { hour: '2-digit', minute: '2-digit' },
                    )}{' '}
                    ngày{' '}
                    {new Date(reschedulingOrder.scheduledAt).toLocaleDateString(
                      'vi-VN',
                    )}
                  </span>
                </div>
                ...
                <div className='space-y-2 pt-1'>
                  ...
                  {isLoadingSlots ? (
                    ...
                    <p className='text-xs text-amber-500 font-bold bg-amber-50 p-3 border border-amber-200 rounded-xl text-center'>
                      Không có ca trống nào vào ngày này. Vui lòng chọn ngày
                      khác.
                    </p>
                  ) : (
                ...
                <div className='space-y-1'>
                  ...
                  <p className='text-xs text-muted-foreground leading-normal'>
                    ...
                    <span className='font-bold text-foreground'>
                      {new Date(cancellingOrder.scheduledAt).toLocaleTimeString(
                        'vi-VN',
                        { hour: '2-digit', minute: '2-digit' },
                      )}{' '}
                      ngày{' '}
                      {new Date(cancellingOrder.scheduledAt).toLocaleDateString(
                        'vi-VN',
                      )}
                    </span>{' '}
                    sẽ bị hủy bỏ.
