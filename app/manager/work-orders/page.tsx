...
export default function ManagerWorkOrdersPage() {
  ...
  const { data: workOrdersRes, isLoading: isLoadingWO, refetch } = useQuery({
    queryKey: ['manager-work-orders', statusFilter],
    queryFn: () => adminGetWorkOrders(statusFilter !== 'all' ? { status: statusFilter } : {}),
  });
  ...
  const workOrders: WorkOrderData[] = workOrdersRes?.data?.data ?? workOrdersRes?.data ?? [];
  const washers: UserData[] = washersRes?.data?.data ?? washersRes?.data ?? [];
  ...
  const qcMutation = useMutation({
    mutationFn: async ({ woId, passed, note }: { woId: string; passed: boolean; note: string }) => {
      ...
      if (passed && qcTarget?.orderId?._id) {
        await adminUpdateOrderStatus(qcTarget.orderId._id, 'completed');
      }
    },
    onSuccess: () => {
      ...
      <main className='flex-1 p-8 overflow-y-auto bg-slate-50/50'>
        <div className='max-w-7xl mx-auto'>
          ...
          {isLoadingWO ? (
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>
              {workOrders.map((wo) => {
                const customer = wo.orderId?.userId?.fullName ?? wo.orderId?.customerName ?? 'Khách vãng lai';
                const plate = wo.orderId?.vehicleId?.licensePlate ?? wo.orderId?.licensePlate ?? '—';
                const service = wo.orderId?.serviceTypeId?.name ?? wo.orderId?.serviceName ?? '—';
                const washer = wo.washerId?.fullName ?? wo.washerId?.name;
                ...
                return (
                  <div
                    ...
                    className={`bg-white rounded-2xl border transition-all duration-200 shadow-sm hover:shadow-md flex flex-col p-6 relative overflow-hidden ${
                      wo.status === 'pending' ? 'border-amber-100 hover:border-amber-200' :
                      wo.status === 'in_progress' ? 'border-indigo-100 hover:border-indigo-200 ring-2 ring-indigo-500/5' :
                  ...
                  >
                    ...
                    <div className='flex items-center justify-between mb-4'>
                      ...
                      <span className={`inline-flex px-2.5 py-0.5 rounded-lg text-xs font-bold ${
                        wo.status === 'pending' ? 'bg-amber-50 text-amber-700 border border-amber-100' :
                        wo.status === 'in_progress' ? 'bg-blue-50 text-blue-700 border border-blue-100 animate-pulse' :
                        'bg-emerald-50 text-emerald-700 border border-emerald-100'
                      }`}>
                        {wo.status === 'pending' && 'Chờ giao thợ'}
                        {wo.status === 'in_progress' && 'Đang rửa xe'}
                        {wo.status === 'completed' && 'Đã rửa xong'}
                      </span>
                    </div>
                    ...
                    <div className='flex-1 flex flex-col gap-2.5 mb-6'>
                      <div>
                        <p className='text-slate-400 text-[10px] uppercase font-bold tracking-wider'>Khách hàng</p>
                      ...
                      {wo.qcStatus && (
                        <div className='mt-2 bg-slate-50 border border-slate-100 p-2 rounded-xl text-xs'>
                          <p className='font-semibold text-slate-700 flex items-center gap-1'>
                            ...
                            QC lần trước: {wo.qcStatus === 'passed' ? 'ĐẠT' : 'KHÔNG ĐẠT'}
                          </p>
          ...
          <div className='bg-white rounded-3xl border border-slate-100 shadow-2xl p-6 max-w-md w-full animate-in fade-in zoom-in-95 duration-150'>
            <div className='flex items-center gap-3 mb-4 text-indigo-600'>
              ...
              <div>
                ...
                <p className='text-xs text-slate-400'>Chọn thợ rửa xe phụ trách cho xe {assignTarget.orderId?.vehicleId?.licensePlate ?? assignTarget.orderId?.licensePlate}</p>
              </div>
            </div>
            ...
            <div className='flex gap-3 justify-end'>
              ...
              <button
                ...
                onClick={() => assignWasherMutation.mutate({ woId: assignTarget._id, washerId: selectedWasherId })}
          ...
          <div className='bg-white rounded-3xl border border-slate-100 shadow-2xl p-6 max-w-md w-full animate-in fade-in zoom-in-95 duration-150'>
            <div className='flex items-center gap-3 mb-5 text-emerald-600'>
              ...
              <div>
                ...
                <p className='text-xs text-slate-400'>Đánh giá kiểm định xe {qcTarget.orderId?.vehicleId?.licensePlate ?? qcTarget.orderId?.licensePlate}</p>
              </div>
            </div>
            ...
            <div className='flex gap-3 justify-end'>
              ...
              <button
                ...
                onClick={() => qcMutation.mutate({ woId: qcTarget._id, passed: qcPassed, note: qcNote })}
