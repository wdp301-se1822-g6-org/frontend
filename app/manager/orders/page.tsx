              ...
              <table className='w-full text-sm text-slate-600'>
                <thead>
                  <tr className='bg-slate-50/50 border-b border-slate-100'>
                    {['ID', 'Khách hàng', 'Biển số', 'Dịch vụ', 'Ca / Ngày hẹn', 'Số tiền', 'Trạng thái', 'Check-in'].map((h) => (
                      <th key={h} className='text-left px-5 py-3.5 text-[11px] font-black uppercase tracking-widest text-slate-400'>
                        {h}
                      </th>
                    ))}
                  </tr>
                      ...
                      return (
                        <tr key={orderId} className='hover:bg-slate-50/20 transition-colors'>
                          ...
                          <td className='px-5 py-4 text-slate-500 text-xs'>
                            ...
                            <div className='text-slate-400'>{(o.scheduledAt ?? o.bookingDate) ? new Date((o.scheduledAt ?? o.bookingDate) as string).toLocaleDateString('vi-VN') : '—'}</div>
                          </td>
                          ...
                          <td className='px-5 py-4'>
                            {isConfirmed ? (
                              ...
                              <span className='text-xs font-medium text-slate-400 flex items-center gap-1 italic'>
